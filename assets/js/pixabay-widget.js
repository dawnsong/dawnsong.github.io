...
(function () {
  ...
  if (typeof pixabayWidget === "object") {
    for (var k in pixabayWidget) {
      if (Object.prototype.hasOwnProperty.call(pixabayWidget, k))
        o[k] = pixabayWidget[k];
    }
  }
...
//   console.log(str)
  ...
  styles = (
    ...
    "**.flex_grid .item img:hover { opacity: .85; }"
  ).replace(/\*\*/g, "." + o.class_name);
  var el = document.createElement("style");
  el.type = "text/css";
  if (el.styleSheet)
    el.styleSheet.cssText = styles; //IE
  ...
  var flexImages = (function () {
    function e(e) {
      function t(e, r, n, i) {
        function o(e) {
          (n.maxRows && g > n.maxRows) || (n.truncate && e && g > 1)
            ? (c[a][0].style.display = "none")
            : (c[a][4] &&
                (c[a][3].setAttribute("src", c[a][4]), (c[a][4] = "")),
              (c[a][0].style.width = l + "px"),
              (c[a][0].style.height = h + "px"),
              (c[a][0].style.display = "block"));
        }
        for (
          var a,
            l,
          ...
          if ((c.push(r[w]), (m += r[w][2] + n.margin), m >= u)) {
            var p = c.length * n.margin;
            for (
              f = (u - p) / (m - p),
                h = Math.ceil(n.rowHeight * f),
                s = 0,
                l,
                a = 0;
              a < c.length;
        ...
        for (a = 0; a < c.length; a++)
          (l = Math.floor(c[a][2] * f)),
            (d = Math.floor(n.rowHeight * f)),
            o(!0);
        ...
        for (
          ...
          if (l.length) {
            var f = window.getComputedStyle
              ...
              : l[0].currentStyle;
            r.margin =
              (parseInt(f.marginLeft) || 0) +
              (parseInt(f.marginRight) || 0) +
              (Math.round(parseFloat(f.borderLeftWidth)) || 0) +
              (Math.round(parseFloat(f.borderRightWidth)) || 0);
            ...
            document.addEventListener
              ? ((window["flexImages_listener" + d] = w),
                window.removeEventListener(
                  "resize",
                  window["flexImages_listener" + a.getAttribute("data-flex-t")]
                ),
                delete window[
                  "flexImages_listener" + a.getAttribute("data-flex-t")
                ],
                window.addEventListener(
                  "resize",
                  window["flexImages_listener" + d]
                ))
              : (a.onresize = w),
  ...
  APIResponse = function (data, n, page, per_page, url) {
    ...
    if (tr == "false") tr = 0;
    else if (tr == "true") tr = 1;
    if (br == "false") br = 0;
    else if (br == "true") br = 1;
    ...
    if (data != false) {
      ...
      for (var i = 0, hits = data.hits; i < hits.length; i++) {
        var w = hits[i].previewWidth,
          h = hits[i].previewHeight,
          src = hits[i].previewURL;
        if (rh > h - 10)
          (w = w * (180 / (h + 1))),
            (h = 180),
            (src = src.replace("_150", "__180"));
        html +=
          '<div class="item" data-w="' +
          w +
          '" data-h="' +
          h +
          '"><a title="' +
          escapeHTML(toTitleCase(hits[i].tags)) +
          '" href="' +
          hits[i].pageURL +
          '" target="' +
          target +
          '"><img src="https://pixabay.com/static/img/blank.gif" data-src="' +
          src +
          '"></a></div>';
      }
  ...
  function closest(el, selector) {
    ...
    while (el) {
      if (match.bind(el)(selector)) return el;
  ...
  function attrs_to_str(n) {
    ...
    for (var i = 0, attrs = n.attributes; i < attrs.length; i++)
      if (attrs[i].name != "data-attrstr" && attrs[i].name != "data-prefilled")
  ...
  function init() {
    for (
      var i = 0, widgets = document.querySelectorAll("." + o.class_name);
      i < widgets.length;
      i++
    ) {
      var n = widgets[i];
      ...
      if (attrs_to_str(n) != n.getAttribute("data-attrstr")) {
        ...
        var url =
          ...
          o.key +
          "&lang=" +
          (n.getAttribute("data-lang") || o.lang) +
          "&order=" +
          (n.getAttribute("data-order") || o.order) +
          "&image_type=" +
          (n.getAttribute("data-image-type") || o.image_type) +
          "&safesearch=" +
          (n.getAttribute("data-safesearch") || o.safesearch) +
          "&editors_choice=" +
          (n.getAttribute("data-editors-choice") || o.editors_choice) +
          "&per_page=" +
          per_page +
          "&page=" +
          page +
        ...
        if (n.getAttribute("data-prefilled")) {
          n.removeAttribute("data-prefilled");
          APIResponse(false, n, page, per_page, url);
        } else if (url in cache)
          APIResponse(cache[url], n, page, per_page, url);
        else {
          var script = document.createElement("script");
          script.src =
            url +
            "&callback=" +
            callback_name(APIResponse, n, page, per_page, url);
